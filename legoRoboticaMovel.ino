...
//Selecionar tarefas
  ...
  if(lcd_key == 3){
    ...
    while(lcd_key != 1){
      ...
      lcd_key = read_LCD_buttons();
      //Mudar com a parte de identificação de cores
  ...
  if(lcd_key == 4){
    ...
    lcd.setCursor(0,0);
    lcd.print("Executando...");
    //Para teste dos movimentos sem o sensor:
    //IdentificouBloco=SELECT; Cor do mesmo > Azul=LEFT; Amarelo=DOWN; Vermelho=UP; Verde=RIGHT.
    tempo_anterior = 0;
    while(sair != 1){
      ...
      //Bloco à frente identificado (SELECT)
      if(lcd_key == 1){
        //Para o robô
        myMotor->run(RELEASE);
            myMotor2->run(RELEASE);
        ...
        //Para teste
        //Azul=LEFT; Amarelo=DOWN; Vermelho=UP; Verde=RIGHT.
        while((lcd_key != 2)&&(lcd_key != 3)&&(lcd_key != 4)&&(lcd_key != 5)){
          lcd_key = read_LCD_buttons();
        }
        ...
        if(lcd_key == 2){
          ...
          //Atualiza tempo de referência
          tempo_anterior = millis();
          //O entendimento do bloco verde pode ser discutido.
          //Como indica uma parada, entendi como fim do percurso,
          //porém como não é claro optei por não fazer 'sair = 1'
          //e sim esperar os 10s.
        }
